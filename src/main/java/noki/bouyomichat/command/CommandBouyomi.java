...
//                    BouyomiChatCore.log("on bouyomi command {}.", Environment.get().getDist().toString());
                    try {
                        PlayerEntity player = commander.asPlayer();
                    }
...
//                Commands.literal("bouyomi")
                        .then(Commands.literal(PAUSE.getName()).executes(context ->  {
                            ...
                            context.getSource().asPlayer().sendMessage(new TranslationTextComponent("bouyomichat.command."+PAUSE.getName()));
                            return 0;
                        }))
                        .then(Commands.literal(RESUME.getName()).executes(context ->  {
                            BouyomiGate.instance.stackQueue(RESUME);
                            context.getSource().asPlayer().sendMessage(new TranslationTextComponent("bouyomichat.command."+RESUME.getName()));
                            return 0;
                        }))
                        .then(Commands.literal(SKIP.getName()).executes(context ->  {
                            BouyomiGate.instance.stackQueue(SKIP);
                            context.getSource().asPlayer().sendMessage(new TranslationTextComponent("bouyomichat.command."+SKIP.getName()));
                            return 0;
                        }))
                        .then(Commands.literal(RELOAD.getName()).executes(context ->  {
                            BouyomiGate.instance.stackQueue(RELOAD);
                            context.getSource().asPlayer().sendMessage(new TranslationTextComponent("bouyomichat.command."+RELOAD.getName()));
                            return 0;
                        }))
                        .then(Commands.literal(CLEAR.getName()).executes(context ->  {
                            BouyomiGate.instance.stackQueue(CLEAR);
                            context.getSource().asPlayer().sendMessage(new TranslationTextComponent("bouyomichat.command."+CLEAR.getName()));
                            return 0;
                        }))
                        /*
                        .then(Commands.literal(EDU.getName())
                                .then(Commands.literal("from")
                                        .then(Commands.literal("to").executes(context -> {
                                            ...
                                            context.getSource().asPlayer().sendMessage(new TranslationTextComponent("bouyomichat.command."+EDU.getName()));
                                            return 0;
                                        }))
                                )
                        )
                        .then(Commands.literal(FORGET.getName())
                                .then(Commands.literal("target").executes(context -> {
                                    ...
                                    context.getSource().asPlayer().sendMessage(new TranslationTextComponent("bouyomichat.command."+FORGET.getName()));
                                    return 0;
                                }))
                        )
                        */
                        .then(Commands.literal(ON.getName()).executes(context ->  {
                            ...
                            BouyomiGate.instance.stackQueue(CLEAR);
                            context.getSource().asPlayer().sendMessage(new TranslationTextComponent("bouyomichat.command."+ON.getName()));
                            return 0;
                        }))
                        .then(Commands.literal(OFF.getName()).executes(context ->  {
                            BouyomiChatConf.currentReadswitchFlag = true;
                            BouyomiChatConf.currentReadSwitch = false;
                            context.getSource().asPlayer().sendMessage(new TranslationTextComponent("bouyomichat.command."+OFF.getName()));
                            return 0;
                        }))
                        .then(Commands.literal(HELP.getName()).executes(context ->  {
                            context.getSource().asPlayer().sendMessage(new TranslationTextComponent("bouyomichat.command."+HELP.getName()));
                            return 0;
                        }))
                        .then(Commands.literal(EXEC.getName()).executes(context ->  {
                            boolean res = BouyomiChatCore.executeBouyomiChan();
                            if(res == true) {
                                context.getSource().asPlayer().sendMessage(new TranslationTextComponent("bouyomichat.command.exec"));
                            }
                            else {
                                context.getSource().asPlayer().sendMessage(new TranslationTextComponent("bouyomichat.command.execfail"));
                            }
                            return 0;
                        }))
        );
